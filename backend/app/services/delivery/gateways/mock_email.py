...
class MockEmailGateway(DeliveryChannelGateway):
    channel = "email"
    ...
    async def send(
        ...
        if config.get("always_fail") or task.metadata.get("mock_always_fail"):
            raise DeliveryChannelError("Mock email permanent failure", retryable=False)
        ...
        if attempt <= failures_before_success:
            raise DeliveryChannelError(f"Mock email temporary failure on attempt {attempt}")
